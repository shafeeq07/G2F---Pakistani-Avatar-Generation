...
class UGATIT(object) :
    ...
    def generate_b2c(self, x_B, reuse=False):
        out, cam, _ = self.generator(x_B, reuse=reuse, scope="generator_A")
    ...
    def build_model(self):
        if self.phase == 'train' :
            ...
            trainC = tf.data.Dataset.from_tensor_slices(self.trainC_dataset)
            ...
            trainC = trainC.apply(shuffle_and_repeat(self.dataset_num)).apply(map_and_batch(Image_Data_Class.image_processing, self.batch_size, num_parallel_batches=16, drop_remainder=True)).apply(prefetch_to_device(gpu_device, None))


            trainA_iterator = trainA.make_one_shot_iterator()
            trainB_iterator = trainB.make_one_shot_iterator()
            trainC_iterator = trainC.make_one_shot_iterator()

            self.domain_A = trainA_iterator.get_next()
            self.domain_B = trainB_iterator.get_next()
            self.domain_C = trainC_iterator.get_next()
            ...
            if self.gan_type.__contains__('wgan') or self.gan_type == 'dragan' :
                GP_A, GP_CAM_A = self.gradient_panalty(real=self.domain_C, fake=x_ba, scope="discriminator_A")
            ...
            reconstruction_A = L1_loss(x_aba, self.domain_C) # reconstruction
